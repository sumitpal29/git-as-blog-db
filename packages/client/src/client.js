...
export function createBlogClient(config) {
  ...
  return {
     ...
     */
    async getBooks() {
      return cachedJson('books/index.json').then(data => data.books ?? []);
    },
     ...
     * @returns {Promise<BookMap>}
     */
    async getBookMap(bookSlug) {
      if (!bookSlug || typeof bookSlug !== 'string') {
        throw new Error('[getBookMap] bookSlug must be a non-empty string');
      }
      return cachedJson(`books/${bookSlug}/map.json`);
    },

    /**
     * Fetch the raw markdown content of a single book file.
     * Parses front-matter if present.
     *
     * @param {string} bookSlug
     * @param {string} filePath - Relative path inside the book (e.g. "concepts/overview.md")
     * @returns {Promise<BookFile>}
     */
    async getBookFile(bookSlug, filePath) {
      if (!bookSlug || typeof bookSlug !== 'string') {
        throw new Error('[getBookFile] bookSlug must be a non-empty string');
      }
      if (!filePath || typeof filePath !== 'string') {
        throw new Error('[getBookFile] filePath must be a non-empty string');
      }
      const normalised = filePath.endsWith('.md') ? filePath : `${filePath}.md`;
      const { frontmatter, content } = await cachedMarkdown(`books/${bookSlug}/${normalised}`);
      return { bookSlug, path: normalised, frontmatter, content };
    },
  };
}
